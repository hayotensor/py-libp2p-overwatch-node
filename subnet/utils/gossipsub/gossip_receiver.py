...
class GossipReceiver:
    ...
    async def _handle_heartbeat(self, message: rpc_pb2.Message, from_peer: str) -> None:
        ...
        subnet_epoch = heartbeat_data.epoch

        # Clear cache on epoch change
        if self._last_epoch != overwatch_epoch:
            logger.debug(
                f"Clearing heartbeat cache for epoch change: {self._last_epoch} -> {overwatch_epoch}"  # noqa: E501
            )
            self._seen_heartbeats.clear()
            self._last_epoch = overwatch_epoch

        key = f"{overwatch_epoch}:{from_peer}"
        ...
        if self.db.nmap_get(nmap, key) is not None:
            logger.debug(f"Heartbeat already exists: {key}")
            return

        # Store it
        self.db.nmap_set(nmap, key, message.data.decode("utf-8"))
        logger.debug(
            f"Heartbeat stored: {nmap}:{key} for node ID {heartbeat_data.subnet_node_id}"  # noqa: E501
        )
