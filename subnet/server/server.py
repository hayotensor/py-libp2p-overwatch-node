...
class Server:
    ...
    async def run(self):
        ...
        cast("Swarm", self.host.get_network()).connection_config.max_connections_per_peer = 10
        # Log available protocols
        logger.info(f"Host ID: {self.host.get_id()}")
        logger.info(
            f"Host multiselect protocols: {self.host.get_mux().get_protocols() if hasattr(self.host, 'get_mux') else 'N/A'}"
        )
        ...
        async with self.host.run(listen_addrs=listen_addrs), trio.open_nursery() as nursery:
            ...
            pubsub = Pubsub(self.host, gossipsub)
            ...
            async with background_trio_service(self.dht):
                ...
                nursery.start_soon(demonstrate_random_walk_discovery, self.dht, 30)
        ...
        print(f"Application shutdown complete for {self.host.get_id()} on subnet {self.subnet_id}")
