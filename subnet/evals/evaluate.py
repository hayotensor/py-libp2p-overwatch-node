...
from subnet.utils.crypto.store_key import get_key_pair, store_key
...
class SubnetEvaluator:
    ...
    async def score_consensus_accuracy(
        ...
        validator_nodes = self.hypertensor.get_min_class_subnet_nodes_formatted(
            self.subnet_id, 0, SubnetNodeClass.Included
        )
        ...
        expected_peer_ids = [PeerID.from_base58(peer_id) for peer_id in expected_peer_ids]
        ...
        for peer_id in expected_peer_ids:
            ...
            if addr is not None:
                try:
                    await connect_to_bootstrap_node_with_retry(
                        host=self.dht.host, bootstrap_addrs=addr, max_retries=3, retry_delay=0.5
                    )
            ...
            for addr in peer_info.addrs:
                ...
                try:
                    await connect_to_bootstrap_node_with_retry(
                        host=self.dht.host, bootstrap_addrs=str(addr), max_retries=3, retry_delay=0.5
                    )
                    successful_connections += 1
...
def get_or_create_key_pair(
    private_key_path: str, hypertensor: Hypertensor, subnet_id: int, overwatch_node_id: int
) -> KeyPair:
    """
    ...
    """
    try:
        return get_key_pair(private_key_path)
    except Exception:
        ...
        hypertensor.set_overwatch_node_peer_id(
            subnet_id=subnet_id, overwatch_node_id=overwatch_node_id, peer_id=peer_id.to_base58()
        )
